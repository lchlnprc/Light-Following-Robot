...
STATE travel_to_fire() {
    ...
    while (true) {
        ...
        if (_closePhototransistor < 700){ //VALUE TO BE TUNED THIS NEEDS TO BE ACCURATE
            if (front_left < 7 && y_left < 6){
                reverse();
                delay(1000);
                stop();
                return FIND_CLOSEST_FIRE;
            }
            if (front_right < 7 && y_right < 6){
                reverse();
                delay(1000);
                stop();
                return FIND_CLOSEST_FIRE;
            }
            if(x_ultrasonic < 7 || front_left < 7 && y_right > 20){
                x_error = 0;
                y_error = -100;
            }
            else if(x_ultrasonic < 7 || front_left < 7 && y_right < 20){
                x_error = 0;
                y_error = 100;
            }
            else if(front_right < 7 && y_left > 20){
                x_error = 0;
                y_error = 100;
            }    
            else if(front_right < 7 && y_left < 20){
